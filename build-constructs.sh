...
docker run --rm -v .:/jsii -w /jsii --user "$(id -u):$(id -g)" -e HOME=/tmp -e YARN_CACHE_FOLDER=/tmp/.yarn-cache jsii/superchain:local bash -c "
    ...
    if ! yarn workspace jsii-pacmak run jsii-pacmak --target rust --force-target --outdir ../../output ../../packages/constructs; then
        echo '❌ Failed to generate constructs - stopping here'
        exit 1
    fi
