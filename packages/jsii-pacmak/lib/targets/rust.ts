...
class RustGenerator implements IGenerator {
  ...
  private generateCargoToml(): void {
    ...
    this.code.line(
      'jsii-runtime = { path = "../../../packages/@jsii/rust-runtime" }',
    );
  ...
  private generateRuntimeInit(): void {
    ...
    this.code.line(
      '/// Initialize jsii runtime (call once before using any generated types)',
    );
    this.code.line('pub fn initialize() -> JsiiResult<()> {');
    this.code.line('    init()');
    this.code.line('}');
    this.code.line('');
  ...
  private generateLibRs(): void {
    ...
    this.code.line('');
    this.code.line(
      'use jsii_runtime::{client, init, JsiiClient, Result as JsiiResult};',
    );
  ...
  private generateClassInline(cls: spec.ClassType): void {
    ...
    if (cls.initializer) {
      ...
      this.code.line(
        `    let response = client_guard.create("${cls.fqn}".to_string(), vec![], None, None)?;`,
      );
      this.code.line(
        '    let object_ref = serde_json::to_string(&response.objref)?;',
      );
  ...
  private generateMethodInline(method: spec.Method): void {
    ...
    this.code.line(
      `  pub fn ${methodName}(&self) -> JsiiResult<${returnType}> {`,
    );
    this.code.line('    let client = get_client()?;');
    this.code.line('    let mut client_guard = client.lock().unwrap();');
    this.code.line(
      `    let objref: Value = serde_json::from_str(&self.object_ref)?;`,
    );
    if (returnType === '()') {
      this.code.line(
        `    let _response = client_guard.invoke(objref, "${method.name}".to_string(), vec![])?;`,
      );
      this.code.line('    Ok(())');
    } else {
      this.code.line(
        `    let response = client_guard.invoke(objref, "${method.name}".to_string(), vec![])?;`,
      );
  ...
  private generatePropertyInline(prop: spec.Property): void {
    ...
    this.code.line(
      `  pub fn ${propName}(&self) -> JsiiResult<${propType}> {`,
    );
    this.code.line('    let client = get_client()?;');
    this.code.line('    let mut client_guard = client.lock().unwrap();');
    this.code.line(
    ...
    this.code.line(
      `    let response = client_guard.get(objref, "${prop.name}".to_string())?;`,
    );
    ...
    if (!prop.immutable) {
      ...
      this.code.line(
        `  pub fn ${setterName}(&mut self, value: ${setterType}) -> JsiiResult<()> {`,
      );
      this.code.line('    let client = get_client()?;');
      this.code.line('    let mut client_guard = client.lock().unwrap();');
      this.code.line(
      ...
      if (setterType.startsWith('Box<dyn ')) {
        this.code.line(
          '    // TODO: Handle trait object serialization for property setters',
        );
        this.code.line(
          '    let json_value = Value::Null; // Placeholder for trait objects',
        );
      } else {
        this.code.line(`    let json_value = serde_json::to_value(value)?;`);
      }
      this.code.line(
        `    client_guard.set(objref, "${prop.name}".to_string(), json_value)?;`,
      );
      this.code.line('    Ok(())');
      this.code.line('  }');
    }
