...
class RustGenerator implements IGenerator {
  ...
  private generateCargoToml(): void {
    ...
    this.code.line('jsii-runtime = { path = "../../../packages/@jsii/rust-runtime" }');
  ...
  private generateRuntimeInit(): void {
    this.code.line('// ============================================================================');
    this.code.line('// Runtime Helper Functions');
    this.code.line('// ============================================================================');
    this.code.line('');
    
    this.code.line('/// Initialize jsii runtime (call once before using any generated types)');
    this.code.line('pub async fn initialize() -> JsiiResult<()> {');
    this.code.line('    init().await');
    this.code.line('}');
    this.code.line('');
    
    this.code.line('/// Get the global jsii client');
    this.code.line('fn get_client() -> JsiiResult<Arc<Mutex<JsiiClient>>> {');
    this.code.line('    client()');
    this.code.line('}');
    this.code.line('');
  }
  ...
  private generateLibRs(): void {
    ...
    this.code.line('');
    this.code.line('use jsii_runtime::{client, init, JsiiClient, Result as JsiiResult};');
  ...
  private generateClassInline(cls: spec.ClassType): void {
    ...
    this.code.line('#[derive(Debug, Clone, Serialize, Deserialize, Default)]');
    ...
    if (cls.initializer) {
      this.code.line(
        '  pub async fn new() -> JsiiResult<Self> {',
      );
      this.code.line('    let client = get_client()?;');
      this.code.line('    let mut client_guard = client.lock().unwrap();');
      this.code.line(`    let response = client_guard.create("${cls.fqn}".to_string(), vec![], None, None).await?;`);
      this.code.line('    let object_ref = serde_json::to_string(&response.objref)?;');
      this.code.line('    Ok(Self { object_ref })');
      this.code.line('  }');
  ...
  private generateEnumInline(enm: spec.EnumType): void {
    ...
    (enm.members ?? []).forEach((member, index) => {
  ...
  private generateMethodInline(method: spec.Method): void {
    ...
    this.code.line(
      `  pub async fn ${methodName}(&self) -> JsiiResult<${returnType}> {`,
    );
    this.code.line('    let client = get_client()?;');
    this.code.line('    let mut client_guard = client.lock().unwrap();');
    this.code.line(`    let objref: Value = serde_json::from_str(&self.object_ref)?;`);
    if (returnType === '()') {
      this.code.line(`    let _response = client_guard.invoke(objref, "${method.name}".to_string(), vec![]).await?;`);
      this.code.line('    Ok(())');
    } else {
      this.code.line(`    let response = client_guard.invoke(objref, "${method.name}".to_string(), vec![]).await?;`);
      if (returnType === 'Value' || returnType.startsWith('Box<dyn ')) {
        this.code.line('    Ok(response.result)');
      } else if (returnType === 'String') {
        this.code.line('    Ok(response.result.as_str().unwrap_or("").to_string())');
      } else if (returnType === 'bool') {
        this.code.line('    Ok(response.result.as_bool().unwrap_or(false))');
      } else if (returnType === 'f64') {
        this.code.line('    Ok(response.result.as_f64().unwrap_or(0.0))');
      } else {
        this.code.line('    // TODO: Implement proper type conversion');
        this.code.line('    Ok(serde_json::from_value(response.result).unwrap_or_default())');
      }
    }
    this.code.line('  }');
  }
  ...
  private generatePropertyInline(prop: spec.Property): void {
    ...
    this.code.line(
      `  pub async fn ${propName}(&self) -> JsiiResult<${propType}> {`,
    );
    this.code.line('    let client = get_client()?;');
    this.code.line('    let mut client_guard = client.lock().unwrap();');
    this.code.line(`    let objref: Value = serde_json::from_str(&self.object_ref)?;`);
    this.code.line(`    let response = client_guard.get(objref, "${prop.name}".to_string()).await?;`);
    if (propType === 'Value' || propType.startsWith('Box<dyn ')) {
      this.code.line('    Ok(response.value)');
    } else if (propType === 'String') {
      this.code.line('    Ok(response.value.as_str().unwrap_or("").to_string())');
    } else if (propType === 'bool') {
      this.code.line('    Ok(response.value.as_bool().unwrap_or(false))');
    } else if (propType === 'f64') {
      this.code.line('    Ok(response.value.as_f64().unwrap_or(0.0))');
    } else {
      this.code.line('    // TODO: Implement proper type conversion');
      this.code.line('    Ok(serde_json::from_value(response.value).unwrap_or_default())');
    }
    this.code.line('  }');
    ...
    if (!prop.immutable) {
      ...
      this.code.line(
        `  pub async fn ${setterName}(&mut self, value: ${setterType}) -> JsiiResult<()> {`,
      );
      this.code.line('    let client = get_client()?;');
      this.code.line('    let mut client_guard = client.lock().unwrap();');
      this.code.line(`    let objref: Value = serde_json::from_str(&self.object_ref)?;`);
      if (setterType.startsWith('Box<dyn ')) {
        this.code.line('    // TODO: Handle trait object serialization for property setters');
        this.code.line('    let json_value = Value::Null; // Placeholder for trait objects');
      } else {
        this.code.line(`    let json_value = serde_json::to_value(value)?;`);
      }
      this.code.line(`    client_guard.set(objref, "${prop.name}".to_string(), json_value).await?;`);
      this.code.line('    Ok(())');
      this.code.line('  }');
    }
  }
  ...
  private toRustType(
    ...
    if ('fqn' in typeRef && typeRef.fqn) {
      ...
      if (this.isInterfaceType(typeRef.fqn)) {
        // For now, return Value for trait objects to avoid deserialization issues
        return isReturnType ? 'Value' : `Box<dyn ${typeName}>`;
      }
