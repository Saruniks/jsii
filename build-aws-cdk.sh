...
docker run --rm -v .:/jsii -w /jsii --user "$(id -u):$(id -g)" -e HOME=/tmp -e YARN_CACHE_FOLDER=/tmp/.yarn-cache jsii/superchain:local bash -c "
    ...
    # if ! yarn workspace jsii-pacmak run jsii-pacmak --targets rust --force-target --outdir ../../output ../../packages/aws-cdk-rust-test/packages/@aws-cdk/region-info; then
    #     echo '❌ Failed to generate aws-cdk - stopping here'
    #     exit 1
    # fi

    # aws-cdk-asset-awscli-v1 = { version = "^2.2.237", path = "../aws-cdk-asset-awscli-v1" }
    # aws-cdk-asset-node-proxy-agent-v6 = { version = "^2.1.0", path = "../aws-cdk-asset-node-proxy-agent-v6" }
    # aws-cdk-cloud-assembly-schema = { version = "^44.1.0", path = "../aws-cdk-cloud-assembly-schema" }

    # if ! yarn workspace jsii-pacmak run jsii-pacmak --targets rust --force-target --outdir ../../output ../../packages/cloud-assembly-schema; then
    #     echo '❌ Failed to generate aws-cdk - stopping here'
    #     exit 1
    # fi

    # if ! yarn workspace jsii-pacmak run jsii-pacmak --targets rust --force-target --outdir ../../output ../../packages/awscdk-asset-node-proxy-agent; then
    #     echo '❌ Failed to generate aws-cdk - stopping here'
    #     exit 1
    # fi

    # if ! yarn workspace jsii-pacmak run jsii-pacmak --targets rust --force-target --outdir ../../output ../../packages/awscdk-asset-awscli; then
    #     echo '❌ Failed to generate aws-cdk - stopping here'
    #     exit 1
    # fi

    # if ! yarn workspace jsii-pacmak run jsii-pacmak --targets rust --force-target --outdir ../../output ../../packages/aws-cdk-rust-test/packages/aws-cdk-lib; then
    #     echo '❌ Failed to generate aws-cdk - stopping here'
    #     exit 1
    # fi
